...
trait CSVRowReader[H <: HList] {
  ...
  def from(row: List[String]): Try[H]
}
object CSVRowReader {
  ...
  def instance[H <: HList](f: List[String] => Try[H]) = new CSVRowReader[H] {
    def from(row: List[String]): Try[H] = f(row)
  }
  ...
  implicit def hNilFromCSVRow: CSVRowReader[HNil] = instance {
    ...
    case _   => Success(HNil) // TODO: what will I do here???
  }
